...
beforeEach(async () => {
	...
	_animaltype2 = await AnimalTypeService.createNew("dog", faker.image.avatar());
	_breed2 = await BreedService.createNew(faker.animal.dog(), "dog");
...
test("Check delete function: try to delete should be success", async () => {
	...
	await AnimalService.deleteOne(owner, animal._id);
...
test("Check changeAvatar: updates avatar", async () => {
	...
	const updated = await AnimalService.changeAvatar(owner, animal._id, newAvatar);
